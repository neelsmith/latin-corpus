...
class LatinCorpusCaseInsensitiveSpec extends FlatSpec {
...
""".split("\n").toVector
  ...
  "A LatinCorpus" should "use case-insensitive matching to find analyses for tokens" in {
    val but = lc.tokens.head
    assert(but.analyses.size == 1)
    assert(but.text == "SED")

    val as = lc.tokens(3)
    assert(as.text == "ut")
    assert(as.analyses.size == 1)
    ...
    println(as.analysisUrns(umgr))
    println(but.analysisUrns(umgr))
